...
# logged in users without defined ages and logged out users are given confirmation pages for all adult content
                    if ( $adult_content eq "explicit" && $remote && $remote->is_minor ) {
                        return $adult_content_handler->(
                            DW::Logic::AdultContent->adult_interstitial_path(
        ...
        )
    {
        ...
        if ( $func eq "normal" ) {
            ...
            unless ( $uri =~ m!^/(\w{1,25})(/.*)?$! ) {
                ...
                my $redir = LJ::Hooks::run_hook( "journal_subdomain_redirect_url", $host, $uri );
    ...
    my $redirect_url;
    my ( $alt_uri, $alt_path ) =
        resolve_path_for_uri( $apache_r, $uri, redirect_url_ref => \$redirect_url );
...
sub vgift_trans {
    ...
    return HTTP_NOT_MODIFIED
        if $apache_r->headers_in->{'If-Modified-Since'}
...
"<h1>Invalid Filter</h1><p>Either this reading filter doesn't exist or you are not authorized to view it. Try <a href='$LJ::SITEROOT/login'>checking that you are logged in</a> if you're sure you have the name right.</p>";
        ...
        $html =
            "<h1>Suspended User</h1>" . "<p>The content at this URL is from a suspended user.</p>";
